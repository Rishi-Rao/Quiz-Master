export default {
    ...
    <div>
        ...
        <router-link v-if="$store.state.loggedIn && $store.state.role == 'admin'" to='/admindash'>Admin Dash</router-link>
        ...
        <button class="btn btn-secondary" v-if="$store.state.loggedIn" @click="$store.commit('logout');$router.push('/login')">Logout</button>
    </div>
    `
}